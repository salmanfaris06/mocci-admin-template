...
import { aiProviderKeys, apiSettings, jobs } from "../../../src/server/db/schema";
...
async function getOpenAiApiKey() {
  const [providerKey] = await db.select().from(aiProviderKeys).where(eq(aiProviderKeys.provider, "openai")).limit(1);
  return decryptIfPossible(providerKey?.encryptedApiKey ?? null) ?? requiredEnv("OPENAI_API_KEY");
}

export async function runNextJob(workerId = "default-worker") {
  const [job] = await db.select().from(jobs).where(and(eq(jobs.status, "queued"), inArray(jobs.type, ["process_webhook", "ai_reply"]), lte(jobs.scheduledAt, new Date()))).orderBy(asc(jobs.scheduledAt)).limit(1);
  if (!job) return false;
  ...
  try {
    ...
    if (job.type === "ai_reply") {
      ...
      await runAiReply({ ...(job.payload as { messageId: string; conversationId: string; contactId: string }), evolutionClient, openAiApiKey: await getOpenAiApiKey() });
