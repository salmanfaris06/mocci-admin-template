...
export const modelPricing = pgTable(
  ...
  (table) => [
    uniqueIndex("model_pricing_provider_model_capability_idx").on(
      table.provider,
      table.modelId,
      table.capability,
    ),
...
export const messages = pgTable(
  ...
  (table) => [
    uniqueIndex("messages_evolution_message_id_idx").on(
      table.evolutionMessageId,
    ),
...
export const inboxEvents = pgTable(
  ...
  (table) => [
    ...
    index("inbox_events_conversation_created_at_idx").on(
      table.conversationId,
      table.createdAt,
    ),
  ],
...
export const aiAgents = pgTable("ai_agents", {
  ...
  typingIntervalSeconds: integer("typing_interval_seconds")
    .default(6)
    .notNull(),
...
export const aiUsageLogs = pgTable("ai_usage_logs", {
  ...
  inputPricePerMillionSnapshot: numeric("input_price_per_million_snapshot", {
    precision: 12,
    scale: 6,
  }).notNull(),
  outputPricePerMillionSnapshot: numeric("output_price_per_million_snapshot", {
    precision: 12,
    scale: 6,
  }).notNull(),
...
export const jobs = pgTable(
  ...
  (table) => [
    ...
    index("jobs_type_status_scheduled_at_idx").on(
      table.type,
      table.status,
      table.scheduledAt,
    ),
