...
import { triggerAiWhatsAppReply } from "./ai-reply";
import { publishInboxEvent } from "./inbox-events";
import { mapAckToMessageStatus, shouldUpdateStatus } from "./message-status";
import { getWhatsAppAiReplyEligibility } from "./whatsapp-ai-eligibility";
import { getGroupNameFromMetadata, isGroupJid } from "./whatsapp-display";
import {
  getEvolutionAckStatus,
  getEvolutionIsFromMe,
  getEvolutionMessageId,
} from "../../app/api/webhooks/evolution/evolution-message";
...
export async function buildEvolutionWebhookIngestion(
  payload: unknown,
  pathEvent?: string,
) {
  ...
  const baseKey = messageId
    ? `${eventType}:${instance}:${messageId}`
    : `${eventType}:${instance}:${await stablePayloadHash((payload as { raw?: unknown }).raw ?? payload)}`;

  return {
    eventType,
    idempotencyKey: baseKey,
    rawPayload: (payload && typeof payload === "object"
      ? payload
...
export async function processEvolutionWebhookPayload(
  ...
  const eventType = getEventType(payload, pathEvent);
  const meta = (payload as { meta?: { parseError?: string } }).meta;
  const parseError = meta?.parseError;

  const processedMessages = parseError
    ? 0
    : await processMessageEvent(payload, pathEvent);
  const processedUpdates = parseError
    ? 0
